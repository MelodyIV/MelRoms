...
class MelRomsLagSwitch:
    ...
    def create_widgets(self):
        ...
        # Title
        tk.Label(main, text="─── MelRoms Lag Switch v4.9.3 ───",
                font=("Courier", 9, "bold"), fg=MIKU_PINK, bg=MIKU_DARK).pack()

        # Stats row
        self.stat_frame = tk.Frame(main, bg=MIKU_DARK)
        self.stat_frame.pack(fill=tk.X, pady=4)
        self.stat_labels = {}
        for key in ["intercepted", "held", "dropped"]:
            lbl = tk.Label(self.stat_frame, text=f"{key.upper()}: 0",
                           font=("Courier", 7), fg=MIKU_TEAL, bg=MIKU_DARK, width=11)
            lbl.pack(side=tk.LEFT, expand=True)
            self.stat_labels[key] = lbl

        # Target frame
        f_app = tk.LabelFrame(main, text=" [ TARGET ] ",
                              font=("Arial", 6, "bold"), fg=MIKU_PINK, bg=BG_PANEL, padx=4, pady=2)
        f_app.pack(fill=tk.X, pady=3)
        ...
        tk.Entry(target_frame, textvariable=self.app_var, bg=TEXT_BG, fg="#fff",
                 font=("Consolas", 8), relief=tk.FLAT, insertbackground="white",
                 width=20).pack(side=tk.LEFT, padx=(0, 4))

        self.system_wide = tk.BooleanVar(value=False)
        tk.Checkbutton(target_frame, text="🌐 Sys‑wide",
                       variable=self.system_wide, command=self.toggle_system_wide,
                       bg=BG_PANEL, fg=MIKU_TEAL, selectcolor=BG_PANEL,
                       activebackground=BG_PANEL, font=("Arial", 7)).pack(side=tk.LEFT)

        self.warning_label = tk.Label(f_app, text="⚠️ Affects ALL apps!",
                                      fg="orange", bg=BG_PANEL, font=("Arial", 6))
        self.warning_label.pack(fill=tk.X, pady=(1, 0))
        self.warning_label.pack_forget()

        # Presets
        f_pre = tk.LabelFrame(main, text=" [ LAG PRESETS ] ",
                              font=("Arial", 6, "bold"), fg=MIKU_TEAL, bg=BG_PANEL, padx=4, pady=2)
        f_pre.pack(fill=tk.X, pady=3)
        ...
        for d in ["Both", "Outbound", "Inbound"]:
            tk.Radiobutton(dir_frame, text=d, variable=self.dir_var, value=d,
                           ...
                           activebackground=MIKU_DARK, font=("Arial", 7)).pack(side=tk.LEFT, padx=5)

        # Value inputs
        pf = tk.Frame(main, bg=BG_PANEL, pady=4)
        ...
        for label, var in [("DELAY(ms)", self.delay_v), ("HOLD(s)", self.hold_v), ("LOSS(%)", self.loss_v)]:
            ...
            tk.Label(row, text=label, bg=BG_PANEL, fg=MIKU_TEAL,
                     width=10, anchor="w", font=("Courier", 8)).pack(side=tk.LEFT)
            tk.Entry(row, textvariable=var, bg=TEXT_BG, fg="#fff",
                     width=5, justify="center", relief=tk.FLAT, font=("Consolas", 8)).pack(side=tk.RIGHT)

        # Start button
        self.start_btn = tk.Button(main, text="ACTIVATE SYSTEM", command=self.toggle_system,
                                   bg=BTN_BG, fg=MIKU_TEAL, font=("Courier", 10, "bold"),
                                   relief=tk.RAISED, height=1, bd=1)
        self.start_btn.pack(fill=tk.X, pady=4)

        # Log area
        self.log_area = scrolledtext.ScrolledText(main, bg=TEXT_BG, fg=TEXT_FG, height=8,
                                                  font=("Consolas", 7), state=tk.DISABLED, relief=tk.FLAT)
