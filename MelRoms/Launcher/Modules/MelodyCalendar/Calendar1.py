...
class Miku3DAnimator:
    ...
    def draw_hair_strands(self, rot_y, rot_x, back_only=True):
        for strand in self.hair_strands:
            ...
            if len(coords) > 1:
                color = self.hsv_to_hex(self.time * 0.5, 0.7, 1.0) if self.has_event_today else MIKU_TEAL
...
class MikuCalendarApp:
    def __init__(self, root):
        self.root = root
        self.root.title("MelRoms × Hatsune Miku Calendar")
    ...
    def switch_theme(self):
        ...
        global MIKU_TEAL, MIKU_PINK, MIKU_DARK
    ...
    def create_widgets(self):
        ...
        tk.Button(ctrl_frame, text="🌱 LEEK MODE", command=self.toggle_leek, bg=DEFAULT_THEME["btn_bg"], fg=MIKU_PINK).pack(side=tk.LEFT, padx=5)
        
        self.update_miku_quote()

    def refresh_calendar(self):
        self.month_label.config(text=self.current_date.strftime("%B %Y"))
        for widget in self.cal_canvas.winfo_children():
            widget.destroy()
        ...
        for i in range(7):
            self.cal_canvas.grid_columnconfigure(i, weight=1)
    ...
    def refresh_upcoming_events(self):
        ...
        items = [f"{k}: {v[0]['title']}" for k,v in self.events.items()]
        self.events_text.insert("1.0", "Upcoming:\n" + "\n".join(items[:10]))

    def update_miku_quote(self):
        quotes = ["Sing with me!", "Leeks are great!", "Let's go!", "Don't forget your plans!", "I'm helping!"]
