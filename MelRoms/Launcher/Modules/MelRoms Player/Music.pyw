...
class App:
    ...
    def play_file(self, file_dict, index=-1):
        try:
            ...
            # If this file is in the queue, remove it
            ...
            if self.show_search or self.search_results_cache:
                # Store the search results before closing
                ...
                # Find which artist this track belongs to
                found = False
                for i, artist in enumerate(self.library.artists):
                    for j, track in enumerate(artist['tracks']):
                        if track['path'] == path:
                            self.selected_artist_idx = i
                            self.current_track_index = j  # Set to the correct index in the artist's tracks
    ...
    def play_next_track(self):
        # Check if we have a queue first
        if self.play_queue and self.current_queue_index < len(self.play_queue) - 1:
            ...
            # We've reached the end of the queue, clear it
            self.play_queue = []
            self.current_queue_index = -1
            # Then continue with normal playback
            files = self.get_current_files()
            if files and self.current_track_index >= 0:
    ...
    def skip_seconds(self, seconds):
        try:
            ...
            if files and self.current_track_index >= 0:
                ...
                # Try using set_pos instead of play(start=)
                if pygame.mixer.music.get_busy():
                    # If music is playing, just set the position
                    try:
                        pygame.mixer.music.set_pos(new_pos)
                        self.current_track_position = new_pos
                        logging.info(f"Set position to {new_pos}s using set_pos")
                    except:
                        # If set_pos fails, fall back to reload method
                    ...
                    # If not playing, just play from new position
                    if "::" in path:
                        zp, inner = path.split("::")
                        with zipfile.ZipFile(zp, 'r') as z:
                            pygame.mixer.music.load(io.BytesIO(z.read(inner)))
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.handle_events()  # Only call this once!
            ...
            # Update position more frequently for better sync
            if not self.is_paused and pygame.mixer.music.get_busy() and not self.is_skipping:
                ...
                # If the actual position is very different from expected, something's wrong
                if abs(pos - expected_pos) > 2.0:  # More than 2 seconds off
                    # Don't update, keep our tracked position
                    pass
                elif abs(pos - self.current_track_position) > 0.1:
                    # Small, reasonable update
                    self.current_track_position = pos
            
            last_position_update = time.time()
            
            
            # Update Discord with current position every 5 seconds
