...
class AsciiAnimator:
    ...
    def hypernova_collapse(self, t, i, count):
        cx, cy = self.w / 2, self.h / 2
        # Loop every 8 seconds – no more vanishing after the cycle ends
        cycle_t = t % 8.0
        angle_offset = i * 0.1 + t * 2   # continuous rotation across cycles

        MAX_RADIUS = min(self.w, self.h) * 0.7   # stays inside canvas

        if cycle_t < 2.0:
            # Phase 1: small yellow expanding core
            pct = cycle_t / 2.0
            seed = i * 13.57
            ...
            val = max(0.1, pct)          # never reaches zero
...
class LauncherApp:
    def __init__(self, root):
        ...
        self.root.after(100, self.load_programs_and_modules)

    def load_programs_and_modules(self):
        self.scan_local_py_files()   
        self.refresh_list()
        self.scan_modules_for_buttons()
    ...
    def remove_program(self):
        ...
        if not selected:
            ...
            return
        item = selected[0]
        prog_name = self.tree.item(item)["values"][0]
        prog_path = self.tree.item(item)["values"][1]
        prog_to_remove = None
    ...
    def create_widgets(self):
        ...
        # Vertical volume slider (above mute button)
        self.volume_slider = tk.Scale(self.left_frame, from_=0, to=100, orient=tk.VERTICAL,
                                      command=self.set_volume, relief=tk.FLAT, length=80)
    ...
    def apply_theme(self, theme_filename):
        ...
        f_main, f_mono = tuple(t["font_main"]), tuple(t["font_mono"])
        self.root.configure(bg=t["bg_root"])

        for fr in (self.main_frame, self.left_frame, self.middle_frame, self.top_rect,
                   self.bottom_container, self.info_frame, self.apps_frame,
                   self.btn_grid, self.anim_container, self.notes_frame):
            fr.configure(bg=t["bg_panel"])

        for b in (self.btn_launch, self.btn_refresh, self.btn_settings, self.btn_add, self.btn_remove):
            b.configure(bg=t["btn_bg"], fg=t["btn_fg"], activebackground=t["btn_active_bg"], activeforeground=t["btn_active_fg"], font=f_main)

        
        self.volume_slider.configure(bg=t["bg_panel"], fg=t["btn_fg"], troughcolor=t["btn_bg"], highlightbackground=t["bg_panel"])
        self.mute_btn.configure(bg=t["mute_active_bg"] if self.is_muted else t["mute_bg"], fg=t["mute_fg"], font=f_main)

        for lf in (self.info_frame, self.apps_frame, self.notes_frame):
            lf.configure(fg=t["info_frame_fg"], font=f_main)

        for txt_widget in (self.info_text, self.notes_text):
            txt_widget.configure(bg=t["info_text_bg"], fg=t["info_text_fg"], font=f_mono, insertbackground=t["info_text_fg"])

        self.animator.set_bg_color(t["bg_anim_frame"])
        style = ttk.Style()
        style.theme_use('default')
        style.configure("Treeview", background=t["tree_bg"], foreground=t["tree_fg"], fieldbackground=t["tree_bg"], borderwidth=0, font=f_main)
        style.map('Treeview', background=[('selected', t["tree_selected_bg"])], foreground=[('selected', t["tree_selected_fg"])])
        style.configure("Treeview.Heading", background=t["tree_header_bg"], foreground=t["tree_header_fg"], font=f_main)

    def scan_modules_for_buttons(self):
        ...
        for col in range(num_columns):
            self.btn_grid.grid_columnconfigure(col, weight=1)
        ...
        for i, folder in enumerate(subdirs):
            ...
            display_name = os.path.splitext(os.path.basename(target_py))[0].upper()
            icon_path = os.path.join(folder_full, "icon.ico")
            img = None
            if os.path.exists(icon_path):
                try:
                    raw_img = Image.open(icon_path).resize((32, 32), Image.Resampling.LANCZOS)
                    img = ImageTk.PhotoImage(raw_img)
                    self.module_icons.append(img)
                except:
                    pass

            btn = tk.Button(self.btn_grid, text=display_name, image=img, compound=tk.TOP, relief=tk.FLAT,
                            command=lambda p=target_py: self.direct_launch(p))
            ...
            btn.configure(bg=t["btn_bg"], fg=t["btn_fg"], activebackground=t["btn_active_bg"],
                          activeforeground=t["btn_active_fg"], font=tuple(t["font_main"]))
    ...
    # ------------------------------------------------------------------
    # NON‑RECURSIVE SCAN: only files directly in the launcher's folder
    # ------------------------------------------------------------------
    def scan_local_py_files(self):
        ...
        # Only look at files in base_dir, not subfolders
