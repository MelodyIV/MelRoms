...
def spawn_key_cmd(entry_widget=None):
    """
    生成 64 位安全随机字符串，并在新 CMD 窗口中显示。
    如果提供了 entry_widget，还会将生成的密钥填入该输入框。
    """
...
class AESCuteObfuscator:
    ...
    def setup_ui(self):
        ...
        self.main_frame = tk.Frame(self.root, bg=self.colors["card"], padx=20, pady=20,
                                   highlightbackground=self.colors["lavender"], highlightthickness=2)
        self.main_frame.pack(padx=30, pady=5, fill="both", expand=True)
        ...
        for text, val in methods:
            rb = tk.Radiobutton(method_frame, text=text, variable=self.obf_method, value=val,
                                ...
                                font=("Segoe UI", 9), selectcolor=self.colors["lavender"])
        ...
        self.key_entry = tk.Entry(key_frame, font=("Segoe UI", 11), bg=self.colors["lavender"],
        ...
        self.start_btn = tk.Button(self.root, text="🚀 Scramble My Code!", font=self.btn_font,
                                   ...
                                   relief="flat", padx=30, pady=12, command=self.execute_obfuscation)
        self.start_btn.pack(pady=10)

        tk.Button(self.root, text="Clear All", font=self.label_font, bg=self.colors["bg"],
                  fg=self.colors["text"], relief="flat", command=self.clear_all).pack()
    ...
    def create_section(self, instruction, btn_text, command, label_attr):
        ...
        btn = tk.Button(self.main_frame, text=btn_text, font=self.btn_font,
                        bg=self.colors["lavender"], fg=self.colors["text"],
                        relief="flat", pady=5, command=command)
        btn.pack(fill="x", pady=(5, 2))
        lbl = tk.Label(self.main_frame, text="No file chosen...", font=("Consolas", 8),
                       bg=self.colors["card"], fg="#AAAAAA", wraplength=350)
