...
class AESCuteRunner:
    ...
    def setup_ui(self):
        tk.Label(self.root, text="✨ AES Unscrambler ✨", font=self.title_font, bg=self.colors["bg"], fg=self.colors["text"], pady=20).pack()
        self.main_frame = tk.Frame(self.root, bg=self.colors["card"], padx=25, pady=25, highlightbackground=self.colors["mint"], highlightthickness=2)
        ...
        tk.Button(self.main_frame, text="Open Protected File", font=self.btn_font, bg=self.colors["lavender"], fg=self.colors["text"], relief="flat", pady=8, command=self.select_file, cursor="hand2").pack(fill="x", pady=(5, 2))
        
        self.file_label = tk.Label(self.main_frame, text="No file selected...", font=("Consolas", 8), bg=self.colors["card"], fg="#AAAAAA", wraplength=350)
        ...
        self.key_entry = tk.Entry(self.main_frame, font=("Segoe UI", 12), bg=self.colors["bg"], fg="white", insertbackground="white", relief="flat", show="*")
        self.key_entry.pack(fill="x", pady=(5, 25), ipady=5)
        self.key_entry.bind("<KeyRelease>", lambda e: self.check_ready())

        self.run_btn = tk.Button(self.root, text="🚀 Decrypt & Run in Memory", font=self.btn_font, bg=self.colors["disabled"], fg="white", state="disabled", relief="flat", padx=30, pady=12, command=self.execute_deobfuscation)
